...
TEST(libpng, mandelbrot)
{
	...
	printf("Saving PNG\n");
	int result = writeImage("mandelbrot.png", width, height, buffer, "This is my test image");
...
float *createMandelbrotImage(int width, int height, float xS, float yS, float rad, int maxIteration)
{
	...
	int xPos, yPos;
	float minMu = static_cast<float>(maxIteration);
	float maxMu = 0;
	...
	for (yPos = 0; yPos<height; yPos++)
	{
		...
		for (xPos = 0; xPos<width; xPos++)
		{
			...
			if (iteration < maxIteration) {
				...
				float mu = static_cast<float>(iteration - (log(log(modZ))) / log(2));
